(function () {
	...
	d3.nest = function () {
		...
		function map(array, depth) {
			...
			var i = -1, n = array.length, key = keys[depth++], keyValue, object, valuesByKey = new d3_Map(),
				values, o = {};
			while (++i < n) {
	...
	d3.format = function (specifier) {
		var match = d3_format_re.exec(specifier), fill = match[1] || " ", align = match[2] || ">",
			sign = match[3] || "", basePrefix = match[4] || "", zfill = match[5], width = +match[6],
			comma = match[7], precision = match[8], type = match[9], scale = 1, suffix = "",
			integer = false;
	...
	d3.ease = function (name) {
		var i = name.indexOf("-"), t = i >= 0 ? name.substring(0, i) : name,
	...
	d3_selectionPrototype.data = function (value, key) {
		...
		function bind(group, groupData) {
			var i, n = group.length, m = groupData.length, n0 = Math.min(n, m), n1 = Math.max(n, m),
				updateNodes = [], enterNodes = [], exitNodes = [], node, nodeData;
			if (key) {
	...
	function d3_transitionNode(node, i, id, inherit) {
		...
		if (!transition) {
			...
			d3.timer(function (elapsed) {
				var d = node.__data__, ease = transition.ease, event = transition.event,
					delay = transition.delay, duration = transition.duration, tweened = [];
	...
	function d3_scale_log(linear, log) {
		...
		scale.ticks = function () {
			...
			if (extent.every(isFinite)) {
				var i = Math.floor(extent[0]), j = Math.ceil(extent[1]), u = pow(extent[0]),
					v = pow(extent[1]);
		...
		function arc() {
			var r0 = innerRadius.apply(this, arguments), r1 = outerRadius.apply(this, arguments),
				...
				a1 = endAngle.apply(this, arguments) + d3_svg_arcOffset, da = (a1 < a0 && (da = a0,
					a0 = a1, a1 = da), a1 - a0), df = da < π ? "0" : "1", c0 = Math.cos(a0),
				s0 = Math.sin(a0), c1 = Math.cos(a1), s1 = Math.sin(a1);
	...
	function d3_svg_lineBasis(points) {
		...
		var i = 1, n = points.length, pi = points[0], x0 = pi[0], y0 = pi[1],
			px = [x0, x0, x0, (pi = points[1])[0]], py = [y0, y0, y0, pi[1]], path = [x0, ",", y0];
	...
	function d3_svg_lineBundle(points, tension) {
		...
		if (n) {
			var x0 = points[0][0], y0 = points[0][1], dx = points[n][0] - x0, dy = points[n][1] - y0,
				i = -1, p, t;
			while (++i <= n) {
				p = points[i];
				t = i / n;
	...
	function d3_svg_area(projection) {
		var x0 = d3_svg_lineX, x1 = d3_svg_lineX, y0 = 0, y1 = d3_svg_lineY, defined = d3_true,
			interpolate = d3_svg_lineLinear, interpolateKey = interpolate.key,
	...
	d3.svg.diagonal = function () {
		...
		function diagonal(d, i) {
			var p0 = source.call(this, d, i), p3 = target.call(this, d, i), m = (p0.y + p3.y) / 2,
				p = [p0, {
					x: p0.x,
					y: m
				}, {
		...
		function axis(g) {
			g.each(function () {
				...
				var lineEnter = tickEnter.select("line"), lineUpdate = tickUpdate.select("line"),
					...
					textEnter = tickEnter.select("text"), textUpdate = tickUpdate.select("text");
				switch (orient) {
					case "bottom": {
	...
	d3.svg.brush = function () {
		...
		function brushstart() {
			var target = this, eventTarget = d3.select(d3.event.target),
				event_ = event.of(target, arguments), g = d3.select(target),
				resizing = eventTarget.datum(), resizingX = !/^(n|s)$/.test(resizing) && x,
	...
	d3.behavior.drag = function () {
		...
		function mousedown() {
			var target = this, event_ = event.of(target, arguments), eventTarget = d3.event.target,
				touchId = d3.event.touches && d3.event.changedTouches[0].identifier, offset,
				origin_ = point(), moved = 0;
	...
	d3.layout.chord = function () {
		...
		function relayout() {
			var subgroups = {}, groupSums = [], groupIndex = d3.range(n), subgroupIndex = [], k, x, x0, i,
				j;
			chords = [];
			groups = [];
			k = 0, i = -1;
	...
	d3.layout.force = function () {
		var force = {}, event = d3.dispatch("start", "tick", "end"), size = [1, 1], drag, alpha, friction = .9,
			linkDistance = d3_layout_forceLinkDistance, linkStrength = d3_layout_forceLinkStrength,
			charge = -30, gravity = .1, theta = .8, interval, nodes = [], links = [], distances, strengths,
			charges;
		
		function repulse(node) {
			return function (quad, x1, y1, x2, y2) {
				if (quad.point !== node) {
					var dx = quad.cx - node.x, dy = quad.cy - node.y,
	...
	var d3_layout_stackOffsets = d3.map({
		...
		wiggle: function (data) {
			var n = data.length, x = data[0], m = x.length, max = 0, i, j, k, s1, s2, s3, dx, o, o0,
				y0 = [];
			y0[0] = o = o0 = 0;
			for (j = 1; j < m; ++j) {
	...
	d3.layout.tree = function () {
		...
		function tree(d, i) {
			...
			function firstWalk(node, previousSibling) {
				...
				if (children && (n = children.length)) {
					var n, firstChild = children[0], previousChild, ancestor = firstChild, child,
						i = -1;
					while (++i < n) {
						child = children[i];
			...
			function apportion(node, previousSibling, ancestor) {
				if (previousSibling) {
					var vip = node, vop = node, vim = previousSibling,
						vom = node.parent.children[0], sip = vip._tree.mod, sop = vop._tree.mod,
						sim = vim._tree.mod, som = vom._tree.mod, shift;
	...
	d3.layout.treemap = function () {
		...
		function squarify(node) {
			var children = node.children;
			if (children && children.length) {
				var rect = pad(node), row = [], remaining = children.slice(), child, best = Infinity,
					score, u = Math.min(rect.dx, rect.dy), n;
	...
	(d3.geo.albers = function () {
		var φ0 = 29.5 * d3_radians, φ1 = 45.5 * d3_radians, m = d3_geo_projectionMutator(d3_geo_albers),
			p = m(φ0, φ1);
	...
	function d3_geo_circleClip(degrees, rotate) {
		var radians = degrees * d3_radians, precision = 6 * d3_radians, normal = [1, 0, 0],
			reference = [0, -1, 0], cr = Math.cos(radians), sr = Math.sin(radians),
			center = d3_geo_circleScale(normal, cr), angle = d3_geo_circleAngle(center);
		return {
		...
		function clipLine(coordinates, context, winding) {
			...
			var point0 = rotate(coordinates[0]), inside = visible(point0),
				keepWinding = inside && winding != null, n;
		...
		function intersect(a, b) {
			...
			var n1 = [1, 0, 0], n2 = d3_geo_circleCross(pa, pb), d1 = center[0], d2 = 0, n1n1 = 1,
				n2n2 = d3_geo_circleDot(n2, n2), n1n2 = n2[0], determinant = n2n2 - n1n2 * n1n2;
			if (!determinant) return a;
			var c1 = (d1 * n2n2 - d2 * n1n2) / determinant, c2 = (d2 * n1n1 - d1 * n1n2) / determinant,
				n1xn2 = d3_geo_circleCross(n1, n2), A = d3_geo_circleScale(n1, c1),
				B = d3_geo_circleScale(n2, c2);
			d3_geo_circleAdd(A, B);
			var u = n1xn2, w = d3_geo_circleDot(A, u), uu = d3_geo_circleDot(u, u),
		...
		function interpolate(from, to, context) {
			...
			var v = d3_geo_circleCross(normal, reference), u0 = reference[0], u1 = reference[1],
				u2 = reference[2], v0 = v[0], v1 = v[1], v2 = v[2];
			for (var step = precision, t = from; t > to; t -= step) {
				var c = Math.cos(t), s = Math.sin(t),
					point = d3_geo_circleSpherical([center[0] + sr * (c * u0 + s * v0), center[1] + sr * (c * u1 + s * v1), center[2] + sr * (c * u2 + s * v2)]);
	...
	function d3_geo_circleClipPolygon(coordinates, context, clipLine, interpolate, angle) {
		...
		coordinates.forEach(function (ring) {
			...
			if (n > 1) {
				var firstSegment = ringSegments[0], lastSegment = ringSegments[n - 1],
					p0 = firstSegment[0], p1 = lastSegment[lastSegment.length - 1];
	...
	d3.geo.greatArc = function () {
		var source = d3_geo_greatArcSource, p0, target = d3_geo_greatArcTarget, p1, precision = 6 * d3_radians,
	...
	function d3_geo_greatArcInterpolator() {
		...
		function interpolate(t) {
			var B = Math.sin(t *= d) * k, A = Math.sin(d - t) * k, x = A * kx0 + B * kx1,
				y = A * ky0 + B * ky1, z = A * sy0 + B * sy1;
	...
	d3.geo.path = function () {
		var pointRadius = 4.5, pointCircle = d3_geo_pathCircle(pointRadius), projection = d3.geo.albersUsa(),
			bounds, buffer = [];
		var bufferContext = {
			point: function (x, y) {
		...
		var centroidType = d3_geo_type({
			...
			GeometryCollection: function (collection) {
				var geometries = collection.geometries, dimensions = geometries.map(geometryDimension),
					dimension = d3.max(dimensions), coordinates = [];
		...
		function multiCentroid(weightedCentroid) {
			return function (o) {
				var coordinates = o.coordinates, centroid, x = 0, y = 0, z = 0, i = -1,
		...
		function lineCentroid(coordinates) {
			...
			var n, point = projection(coordinates[0]), x0 = point[0], y0 = point[1], x1, y1, dx, dy, x = 0,
				y = 0, z = 0, i = 0, δ;
			while (++i < n) {
		...
		function polygonCentroid(coordinates) {
			var polygon = d3.geom.polygon(coordinates[0].map(projection)), area = polygon.area(),
				centroid = polygon.centroid(area < 0 ? (area *= -1,
					1) : -1), x = centroid[0], y = centroid[1], z = area, i = 0,
				n = coordinates.length;
			while (++i < n) {
	...
	function d3_geo_projectionMutator(projectAt) {
		var project, rotate, projectRotate, k = 150, x = 480, y = 250, λ = 0, φ = 0, δλ = 0, δφ = 0, δγ = 0,
			δx = x, δy = y, δ2 = .5, clip = d3_geo_projectionCutAntemeridian(rotatePoint), clipAngle = null;
		...
		function resample(context) {
			...
			function resampleLineTo(x0, y0, λ0, φ0, x1, y1, λ1, φ1, depth) {
				...
				if (distance2 > 4 * δ2 && depth--) {
					var sinφ0 = Math.sin(φ0), cosφ0 = Math.cos(φ0), sinφ1 = Math.sin(φ1),
						cosφ1 = Math.cos(φ1),
						...
						z = k * (sinφ0 + sinφ1), φ2 = Math.asin(Math.max(-1, Math.min(1, z))),
						λ2 = Math.abs(x) < ε || Math.abs(y) < ε ? (λ0 + λ1) / 2 : Math.atan2(y, x),
						p = projectPoint(λ2, φ2), x2 = p[0], y2 = p[1], dx2 = x0 - x2,
						dy2 = y0 - y2, dz = dx * dy2 - dy * dx2,
	...
	function d3_geo_projectionCutAntemeridian(rotatePoint) {
		var clip = {
			...
			line: function (coordinates, context) {
				if (!(n = coordinates.length)) return;
				var point = rotatePoint(coordinates[0]), λ0 = point[0], φ0 = point[1], λ1, φ1,
					sλ0 = λ0 > 0 ? π : -π, sλ1, i = 0, n;
	...
	d3.geom.hull = function (vertices) {
		...
		var len = vertices.length, plen = len - 1, points = [], stack = [], i, j, h = 0, x1, y1, x2, y2, u, v,
			a, sp;
		for (i = 1; i < len; ++i) {
	...
	function d3_geom_polygonIntersect(c, d, a, b) {
		var x1 = c[0], x2 = d[0], x3 = a[0], x4 = b[0], y1 = c[1], y2 = d[1], y3 = a[1], y4 = b[1],
	...
	d3.geom.voronoi = function (vertices) {
		...
		polygons.forEach(function (polygon, i) {
			...
			if (n > 2) return;
			var p0 = vertices[i], p1 = polygon[0], p2 = polygon[1], x0 = p0[0], y0 = p0[1], x1 = p1[0],
				y1 = p1[1], x2 = p2[0], y2 = p2[1], dx = Math.abs(x2 - x1), dy = y2 - y1;
			if (Math.abs(dy) < ε) {
				var y = y0 < y1 ? -Z : Z;
	...
	function d3_voronoi_tessellate(vertices, callback) {
		...
		var Geom = {
			bisect: function (s1, s2) {
				...
				var dx = s2.x - s1.x, dy = s2.y - s1.y, adx = dx > 0 ? dx : -dx,
					ady = dy > 0 ? dy : -dy;
			...
			intersect: function (el1, el2) {
				...
				var xint = (e1.c * e2.b - e2.c * e1.b) / d, yint = (e2.c * e1.a - e1.c * e2.a) / d,
					e1r = e1.region.r, e2r = e2.region.r, el, e;
			...
			rightOf: function (he, p) {
				...
				if (e.a === 1) {
					...
					var yl = e.c - e.a * p.x, t1 = p.y - yl, t2 = p.x - topsite.x,
						t3 = yl - topsite.y;
	...
	function d3_time_scale(linear, methods, format) {
		...
		scale.ticks = function (m, k) {
			...
			if (typeof m !== "function") {
				var span = extent[1] - extent[0], target = span / m,
