d3 = function () {
	...
	var d3_select = function (s, n) {
			...
			try {
				...
				console.log(s.slice(e.position)); // ":foo"
			}
	...
	function d3_geo_albersUsaInvert(projection, extent) {
		...
		return function (coordinates) {
			var dx = coordinates[0] - cx, dy = coordinates[1] - cy, r = dx * dx + dy * dy,
				a = Math.atan2(dy, dx);
	...
	d3.geom.hull = function (vertices) {
		...
		function hull(data) {
			...
			var fx = d3_functor(x), fy = d3_functor(y), n = data.length, vertices, plen = n - 1,
				points = [], stack = [], d, i, j, h = 0, x1, y1, x2, y2, u, v, a, sp;
	...
	d3.geom.voronoi = function (points) {
		...
		function voronoi(data) {
			...
			polygons.forEach(function (polygon, i) {
				...
				if (n > 2) return;
				var p0 = points[i], p1 = polygon[0], p2 = polygon[1], x0 = p0[0], y0 = p0[1],
					x1 = p1[0], y1 = p1[1], x2 = p2[0], y2 = p2[1], dx = Math.abs(x2 - x1),
					dy = y2 - y1;
				if (Math.abs(dy) < ε) {
					var y = y0 < y1 ? -Z : Z;
	...
	function d3_scale_log(linear, base, log, pow) {
		...
		scale.ticks = function () {
			...
			if (extent.every(isFinite)) {
				var b = Math.log(base), i = Math.floor(extent[0] / b), j = Math.ceil(extent[1] / b),
					u = pow(extent[0]), v = pow(extent[1]), n = base % 1 ? 2 : base;
		...
		scale.tickFormat = function (n, format) {
			...
			var b = Math.log(base), k = Math.max(.1, n / scale.ticks().length),
