d3 = function () {
	...
	d3.svg.axis = function () {
		var scale = d3.scale.linear(), orient = d3_svg_axisDefaultOrient, tickMajorSize = 6, tickMinorSize = 6,
			tickEndSize = 6, tickPadding = 3, tickArguments_ = [10], tickValues = null, tickFormat_,
			tickSubdivide = 0;
	...
	d3.behavior.drag = function () {
		...
		function mousedown() {
			var target = this, event_ = event.of(target, arguments), eventTarget = d3.event.target,
				touchId = d3.event.touches ? d3.event.changedTouches[0].identifier : null, offset,
	...
	var d3_behavior_zoomDelta,
		d3_behavior_zoomWheel = "onwheel" in document ? (d3_behavior_zoomDelta = function () {
	...
	d3.layout.force = function () {
		var force = {}, event = d3.dispatch("start", "tick", "end"), size = [1, 1], drag, alpha, friction = .9,
			...
			charge = -30, gravity = .1, theta = .8, nodes = [], links = [], distances, strengths, charges;
	...
	function d3_geo_clipAreaRing(ring, invisible) {
		...
		var n, i = 0, area = 0, p = ring[0], λ = p[0], φ = p[1], cosφ = Math.cos(φ),
			...
			y0 = 1 - invisible * Math.cos(λ) * cosφ, x1 = x0, x, y;
		while (++i < n) {
			p = ring[i];
			cosφ = Math.cos(φ = p[1]);
	...
	function d3_geo_clipCircle(degrees) {
		var radians = degrees * d3_radians, cr = Math.cos(radians),
	...
	d3.geo.greatArc = function () {
		...
		function greatArc() {
			var p0 = source_ || source.apply(this, arguments),
				...
				i = interpolate || d3.geo.interpolate(p0, p1), t = 0, dt = precision / i.distance,
				coordinates = [p0];
	...
	function d3_geo_areaRingStart() {
		...
		function nextPoint(λ, φ) {
			...
			var dλ = λ - λ0, cosφ = Math.cos(φ), sinφ = Math.sin(φ), k = sinφ0 * sinφ,
				u0 = d3_geo_areaRingU, v0 = d3_geo_areaRingV, u = cosφ0 * cosφ + k * Math.cos(dλ),
				v = k * Math.sin(dλ);
	...
	function d3_geo_projectionMutator(projectAt) {
		var project, rotate, projectRotate, projectResample = d3_geo_resample(function (x, y) {
			...
			clip = d3_geo_clipAntimeridian, clipAngle = null;
